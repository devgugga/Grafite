...
This clause applies only while the repository maintains a Graphify
knowledge graph in `graphify-out/`. When it does, the dedicated commit
carrying strictly `graphify-out/**` with title `🕸️ sync knowledge graph`
is the **sole exception** where the mandatory structured body is
omitted. In that two-commit workflow:
