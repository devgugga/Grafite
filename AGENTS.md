...
Grafite is a CLI written in **Rust** that unifies engineering intelligence tooling behind a single interface designed to be driven by AI agents.

- **Primary consumer:** autonomous agents, not humans. Output stability and machine-readability are product requirements, not conveniences.
- **Secondary consumer:** the human engineer operating the same commands interactively.

The repository is at its foundation stage. Architecture decisions that are not yet made are **not** documented as if they were: domain packs start minimal and grow as decisions land.

---

## 2. Agent Architecture: "Domain Packs"
...
| `rust-reviewer` | Antigravity, Claude Code, Codex | Read-Only | Validates Rust and CLI-contract correctness against `docs/domains/rust/`. Reports violations; never edits code. |

Implementation is performed by the main agent. A dedicated implementer subagent is deliberately **not** shipped: it would encode conventions nobody has decided yet.

---

## 4. Fundamental Engineering Guidelines
...
## 5. Agent Workflow & Operational Rules
...
- Specs and plans are archived under [`docs/superpowers/`](./docs/superpowers/README.md).
...
- Run `cargo fmt --check`, `cargo clippy`, `cargo test`, and inspect `git diff` before claiming completion. Never claim success without empirical evidence.

### 5. Human Gatekeeper
- The human engineer is the ultimate decision maker and domain authority.
- Never commit directly to `main`/`master`, apply destructive operations, or run `git push` without explicit human authorization.

---

## 6. Skills: Single Source of Truth & Synchronization
...
## 7. Rust & CLI Conventions

Language, build, and output-contract rules live in [`docs/domains/rust/`](./docs/domains/rust/README.md):
- [`conventions.md`](./docs/domains/rust/conventions.md): toolchain gates, error handling, dependency discipline, unsafe policy.
- [`cli-contract.md`](./docs/domains/rust/cli-contract.md): stream separation, exit codes, machine-readable output, and stability guarantees for agent consumers.

---

## 8. Git & Commit Governance
